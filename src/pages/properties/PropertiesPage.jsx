...
const ALL_COLUMNS = [
  ...
  { key: 'parcel_id',          label: 'Parcel ID',        visible: true },
  { key: 'property_type',      label: 'Property Type',    visible: true },
  { key: 'appeal_prop_type',   label: 'Appeal Prop Type', visible: true },
  { key: 'total_building_sqft',label: 'Bldg SF',          visible: true },
  { key: 'annual_tax_bill',    label: 'Tax Bill',         visible: true },
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 2 }}>
        {prop.property_type && (
          <span style={{ fontSize: 11, padding: '2px 7px', borderRadius: 99, background: `${typeColor}18`, color: typeColor, fontWeight: 600 }}>
            {prop.property_type}
          </span>
        )}
        {prop.appeal_prop_type && (
          <span style={{ fontSize: 11, padding: '2px 7px', borderRadius: 99, background: '#f0fdf4', color: '#16a34a', fontWeight: 600 }}>
            {prop.appeal_prop_type}
          </span>
        )}
        {prop.county && (
          <span style={{ fontSize: 11, padding: '2px 7px', borderRadius: 99, background: '#f8fafc', color: '#64748b', border: '1px solid #e2e8f0' }}>
            {prop.county}
          </span>
        )}
      </div>
  ...
  return (
    <div style={{ padding: 24 }}>
      <div style={{
        ...
        <iframe
          title="Property Map"
          src="https://www.openstreetmap.org/export/embed.html?bbox=-89.9%2C40.5%2C-89.4%2C40.9&layer=mapnik"
          style={{ width: '100%', height: 500, border: 'none' }}
        />
        <div style={{
          position: 'absolute', top: 12, right: 12,
          background: 'rgba(255,255,255,0.95)', borderRadius: 10, padding: '10px 14px',
          boxShadow: '0 4px 12px rgba(0,0,0,0.1)', maxWidth: 260, fontSize: 12, color: '#64748b'
        }}>
      ...
      <div style={{ marginTop: 16, display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 8 }}>
        {properties.slice(0, 10).map(p => (
          <div key={p.id} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 8, padding: '10px 14px', fontSize: 13 }}>
            <div style={{ fontWeight: 600, color: '#1e293b' }}>{p.address || '(No address)'}</div>
            <div style={{ fontSize: 12, color: '#64748b' }}>{p.county} · {p.parcel_id}</div>
          </div>
        ))}
        {properties.length > 10 && (
          <div style={{ background: '#f8fafc', border: '1px dashed #e2e8f0', borderRadius: 8, padding: '10px 14px', fontSize: 13, color: '#64748b', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #e2e8f0', padding: '10px 24px' }}>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          {(county !== 'All' || propertyType !== 'All' || appealPropType !== 'All' || search) && (
            <button onClick={() => { setSearch(''); setCounty('All'); setPropertyType('All'); setAppealPropType('All'); setPage(0) }}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  ...
                  {visibleCols.includes(columns.find(c=>c.key==='address')) && columns.find(c=>c.key==='address')?.visible !== false && <SortTh label="Address" field="address" align="left" />}
                  {columns.find(c=>c.key==='county')?.visible !== false && <SortTh label="County" field="county" />}
                  {columns.find(c=>c.key==='parcel_id')?.visible !== false && <SortTh label="Parcel ID" field="parcel_id" />}
                  {columns.find(c=>c.key==='property_type')?.visible !== false && <SortTh label="Prop Type" field="property_type" />}
                  {columns.find(c=>c.key==='appeal_prop_type')?.visible !== false && <SortTh label="Appeal Type" field="appeal_prop_type" />}
                  {columns.find(c=>c.key==='total_building_sqft')?.visible !== false && <SortTh label="Bldg SF" field="total_building_sqft" />}
                  {columns.find(c=>c.key==='annual_tax_bill')?.visible !== false && <SortTh label="Tax Bill" field="annual_tax_bill" />}
                  {columns.find(c=>c.key==='tax_year')?.visible !== false && <SortTh label="Tax Year" field="tax_year" />}
                  <th style={{ padding:'10px 12px', textAlign:'center', fontSize:12, fontWeight:600, color:'#64748b', textTransform:'uppercase', letterSpacing:'0.05em', backgroundColor:'#f8fafc', borderBottom:'1px solid #e2e8f0' }}>Actions</th>
                </tr>
              </thead>
              <tbody>
                {properties.map((prop, idx) => (
                  <tr key={prop.id} style={{ backgroundColor: selectedIds.includes(prop.id) ? '#eff6ff' : idx % 2 === 0 ? '#f8fafc' : '#fff', borderBottom:'1px solid #e2e8f0' }}>
                    ...
                    {columns.find(c=>c.key==='address')?.visible !== false && (
                      <td style={{ padding:'10px 12px', fontSize:13, textAlign:'left', cursor:'pointer' }} onClick={() => navigate(`/properties/${prop.id}`)}>
                        <span style={{ color:'#1e40af', fontWeight:500 }}>{prop.address || '(No address)'}</span>
                        {prop.city && <span style={{ color:'#94a3b8', marginLeft:6, fontSize:12 }}>{prop.city}</span>}
                      </td>
                    )}
                    {columns.find(c=>c.key==='county')?.visible !== false && <td style={{ padding:'10px 12px', fontSize:13, color:'#1e293b', textAlign:'center' }}>{prop.county || '—'}</td>}
                    {columns.find(c=>c.key==='parcel_id')?.visible !== false && <td style={{ padding:'10px 12px', fontSize:12, color:'#64748b', textAlign:'center', fontFamily:'monospace' }}>{prop.parcel_id || '—'}</td>}
                    {columns.find(c=>c.key==='property_type')?.visible !== false && <td style={{ padding:'10px 12px', fontSize:13, color:'#1e293b', textAlign:'center' }}>{prop.property_type || '—'}</td>}
                    {columns.find(c=>c.key==='appeal_prop_type')?.visible !== false && (
                      <td style={{ padding:'10px 12px', textAlign:'center' }}>
                        {prop.appeal_prop_type
                          ? <span style={{ fontSize:12, padding:'2px 8px', borderRadius:99, background:'#f0fdf4', color:'#16a34a', fontWeight:600 }}>{prop.appeal_prop_type}</span>
                          : <span style={{ color:'#94a3b8', fontSize:13 }}>—</span>}
                      </td>
                    )}
                    {columns.find(c=>c.key==='total_building_sqft')?.visible !== false && <td style={{ padding:'10px 12px', fontSize:13, color:'#1e293b', textAlign:'center' }}>{prop.total_building_sqft ? prop.total_building_sqft.toLocaleString() : '—'}</td>}
                    {columns.find(c=>c.key==='annual_tax_bill')?.visible !== false && <td style={{ padding:'10px 12px', fontSize:13, color:'#1e293b', textAlign:'center' }}>{prop.annual_tax_bill ? fmt.currency(prop.annual_tax_bill) : '—'}</td>}
                    {columns.find(c=>c.key==='tax_year')?.visible !== false && <td style={{ padding:'10px 12px', fontSize:13, color:'#1e293b', textAlign:'center' }}>{prop.tax_year || '—'}</td>}
                    <td style={{ padding:'10px 12px', textAlign:'center' }}>
